...
window.SKILL_TREE_DATA = {
  ...
  changelog: [
    { date: "2026-07-13", title: "Fix: grants permanently failing after one expiry", detail: "\"Couldn't grant — an item may already be under a grant\" fired on items with NO live grant. Root cause: expired grants only get revoked_at via the pg_cron sweep, which isn't installed on every project — so a leftover expired row kept blocking the partial unique index one_live_grant_per_item forever. grant_change_request now reclaims expired-but-unswept grants on the requested items before inserting (no pg_cron dependency). Toast now shows the real server reason.", tokensSaved: 2000, type: "fix", files: ["supabase/migrations/20260713000005_grant_selfheal.sql", "apps/desktop/src/lib/access.ts", "apps/desktop/src/components/AccessCenter.tsx"] },
    { date: "2026-07-13", title: "Personal tree grows like a real tree", detail: "The tree now grows in height AND width: each kind owns an angular sector, the first ~10 items fill the bottom ring, and overflow (#11+) forks a fresh branch one level higher up the trunk. The trunk rebuilds taller and the camera pull-back scales to the crown height, so a big toolkit reads as a tall, branching tree instead of cramming into fixed clusters.", tokensSaved: 3000, type: "feature", files: ["apps/desktop/src/scene/scene.ts"] },
    { date: "2026-07-13", title: "Galaxy: visible write-locks + one-row mobile header", detail: "Live 30-min grants are now visible to everyone (new phase-5 RLS policy) — the item panel shows '🔒 @user is editing — mm:ss left' so a third viewer can tell an item is locked instead of a grant silently failing. Grant query joins the grantee handle. Mobile header collapses tab labels, ✚ Add, and @handle to icons so it fits one row down to ~320px.", tokensSaved: 4000, type: "fix", files: ["supabase/migrations/20260713000004_phase5_grant_visibility.sql", "apps/desktop/src/lib/access.ts", "apps/desktop/src/components/GalaxyTab.tsx", "apps/desktop/src/App.tsx", "apps/desktop/src/components/AuthMenu.tsx"] },
