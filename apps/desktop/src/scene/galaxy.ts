...
export class GalaxyBackground {
  ...
  constructor() {
    ...
    // nebula: a few huge soft billboards tinted brand purple/cyan
    const nebulaTints = [0x2b2f69, 0x1e4a5c, 0x3d2b5e];
    for (let i = 0; i < 5; i++) {
      ...
      s.position.set(Math.cos(a) * 70, 14 + Math.sin(i * 2.7) * 18, Math.sin(a) * 70);
      const size = 55 + (i % 3) * 22;
